...
birth order - variables can be gendered - identity matrix format?
...
## Appended by duke: trying to extract firstborns
...
```
    ...
    Warning in left_join(sib.long, df, by = c(ego = "profileid")): Detected an unexpected many-to-many relationship between `x` and `y`.
    i Row 1407 of `x` matches multiple rows in `y`.
