...
```{r descrtiptives deathage}
summary(data$deathage)
```

```{r descrtiptives migrant}
summary(data$migrant)
```

```{r descrtiptives death_country}
table(data$death_country)
```

```{r descrtiptives birth10}
summary(data$birth10)
```

```{r descrtiptives death10}
summary(data$death10)
```

```{r descrtiptives gender}
summary(data$gender)
```

```{r descrtiptives sib_size_cat}
summary(data$sib_size_cat)
```

```{r descrtiptives famid}
summary(data$famid)
```

```{r descrtiptives firstborn}
...
cov.labs <- c("Migrant", "Country of Death: Canada", "Country of Death: South Africa", "Country of Death: Australia", 
              "Country of Death: New Zealand", "Country of Death: United States of America")
omit.vars <- c("birth10", "sib_size_cat", "gender")

stargazer(fe1, fe2,
                 type = "text",
                 column.labels = c("Migrant Model", "Country of Death Model"),
