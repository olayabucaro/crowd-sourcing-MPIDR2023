...
```{r}
birth.mm <- margins(fe1, variables = "birth10", data = data_par_complete[data_par_complete[["migrant"]] == "migrant",])
birth.mm <- margins(fe1, variables = "birth10", at = list(migrant = c("nonmigrant","migrant")))
birth.mm <- margins(fe1, variables = "migrant", at = list(birth10 = c("1730","1740", "1750", "1760", "1770", "1780", "1790", "1800", "1810", "1820", "1830", "1840", "1850", "1860", "1870", "1880", "1890")))
pb <- summary(birth.mm) # AME = 5.93, p < 0.001
...
plot.pb <- pb %>% 
  ...
  scale_x_discrete(labels=c("1730", "1740", "1750", "1760", "1770", "1780", "1790",
                            "1800", "1810", "1820", "1830", "1840", "1850",
                            "1860", "1870", "1880", "1890"))

ggsave(
  plot = plot.pb,
  filename = "./analysis_output/fe1_birth10.png",
  width = 8.5,
  height = 5,
  units = "in"
)
...
plot.p.sibs <- p %>% 
  ...
  ylab("Average Marginal Effect") + 
  ylim(c(1,11.5)) +
  geom_point(data = p.sibs)+
  geom_errorbar(data = p.sibs, aes(
    ymin = lower,
    ymax = upper), 
    width = 0.15) +
  theme_bw() + 
...
```{r}
birth.mm.sib <- margins(fe1.sibs, variables = "migrant", at = list(birth10 = c("1730","1740", "1750", "1760", "1770", "1780", "1790", "1800", "1810", "1820", "1830", "1840", "1850", "1860", "1870", "1880", "1890")))
pb.sib <- summary(birth.mm.sib) # AME = 5.93, p < 0.001
...
plot.pb.sib <- pb %>% 
  ...
  #ylim(c(1,11.5)) +
  theme_bw() + 
  geom_point(data = pb.sib)+
  geom_errorbar(data = pb.sib, aes(
    ymin = lower,
    ymax = upper), 
    width = 0.15) +
  theme(axis.text.x = element_text(angle=35)) + 
...
ggsave(
  plot = plot.pb.sib,
  filename = "./analysis_output/fe1_birth10.sib.png",
  width = 8.5,
  height = 5,
  units = "in"
)
...
plot.q.sibs <- q %>% 
  ...
  ylab("") + 
  theme_bw()+
  geom_point(data = q.sibs)+
  geom_errorbar(data = q.sibs, aes(
    ymin = lower,
    ymax = upper), 
    width = 0.15) 
...
plot.pq <- ggarrange(plot.p, plot.q, labels = c("A1", "B1"), label.x = 0.5, widths = c(1, 2))


ggsave(
  plot = plot.pq,
  filename = "./analysis_output/fe_arr.png",
  width = 8.5,
  height = 5,
  units = "in"
)
...
plot.pq.sibs <- ggarrange(plot.p.sibs, plot.q.sibs, labels = c("A2", "B2"), label.x = 0.5, widths = c(1, 2))


ggsave(
  plot = plot.pq.sibs,
  filename = "./analysis_output/fe_sib_arr.png",
  width = 8.5,
  height = 5,
  units = "in"
)
