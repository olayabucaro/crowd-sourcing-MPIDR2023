...
Load necessary packages and paths to raw data in N: drive and location to save the cleaned files.

> `Profile` is the dataset with all the individual information. `Links` is the dataset with all the child-parent relations
...
# Load packages
library(data.table)
library(mapdata) # for coordinate matching
...
Read to data.table. Characters that should be read as NA are specified (\*) and encoding set to UTF-8
...
# Calculate precentage of NA for all columns
...
> `Profile` is the dataset with all the individual information. `Links` is the dataset with all the child-parent relations. Select only the observations that are present (as a child or as a parent) in the links dataset
...
## Save `moddata1` data to file (without coordinate matching)

```{r}

fwrite(data, paste0(save_path, "moddata1.csv")) # Data without coordinate matching
...
> Use map.where function from mapdata package: strings naming the region identified by the coordinates. Extract location for births, baptisms, deaths, burials

```{r}

read_coordinates <- function(profiles, birth_coord = TRUE, baptism_coord = TRUE, 
                             burial_coord = TRUE, death_coord = TRUE) {
  ...
  if(birth_coord){
    ...
    profiles$birth_location_longitude = fifelse(is.na(profiles$birth_location_latitude) |
                                                profiles$birth_location_longitude==0,
                                                NA_real_,
...
## Save `moddata2` data to file (With coordinate matching using Low (mid) resolution World Map)

```{r}

fwrite(data_recoded, paste0(save_path, "moddata2.csv")) # Data with coordinate matching
